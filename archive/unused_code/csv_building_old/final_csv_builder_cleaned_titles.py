"""
Final CSV Builder with Cleaned Titles
Uses only works.original_title with simple bracket stripping.
Based on the working optimized builder logic.
"""
...
class FinalCSVBuilderCleanedTitles:
    """
    CSV Builder that uses only works.original_title with simple bracket stripping.
    
    Key features:
    - Uses ONLY works.original_title as title source (no fallbacks)
    - Applies simple bracket/parentheses removal
    - Keeps all working logic from original optimized builder
    - Creates new CSV file (doesn't overwrite existing)
    """
    ...
    def build_final_csv_cleaned_titles(self, sample_size: Optional[int] = None) -> str:
        """
        Build the final CSV using only works.original_title with bracket cleaning.
        ...
        """
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏗️ Building CSV with cleaned titles from works.original_title...")
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏗️ Processing works with title cleaning...")
        master_records = []
        processed_works = 0
        ...
        if sample_size:
            filename = f"final_books_2000_2020_en_cleaned_titles_sampled_{sample_size}_{timestamp}.csv"
        else:
            filename = f"final_books_2000_2020_en_cleaned_titles_{timestamp}.csv"
    ...
    # Copy all the working helper methods from the original optimized builder
