...
class OptimizedFinalCSVBuilder:
    ...
    def build_final_csv_optimized(self, sample_size: Optional[int] = None) -> str:
        """
        Build the final CSV using optimized pandas operations.
        ...
        """
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏗️ Building final CSV with optimized processing...")
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] 🏗️ Processing works in batches...")
        master_records = []
        processed_works = 0
        ...
        for work_id, group in work_groups:
            try:
                ...
                master_record = {
                    ...
                    'title': work_info.get('original_title') or editions[0].get('title', ''),
