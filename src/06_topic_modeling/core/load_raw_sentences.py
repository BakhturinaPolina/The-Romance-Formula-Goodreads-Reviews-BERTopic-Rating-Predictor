...
def load_raw_sentences_from_reviews(
    ...
    # Note: Data loading utilities are now in 05_prepare_reviews_corpus_for_BERTopic
    import sys
    from pathlib import Path
    
    # Try importing from the preparation module
    prep_dir = Path(__file__).parent.parent.parent / "05_prepare_reviews_corpus_for_BERTopic" / "utils"
    if prep_dir.exists() and str(prep_dir) not in sys.path:
    ...
    try:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "data_loading",
                prep_dir / "data_loading.py"
            )
            data_loading = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(data_loading)
            load_joined_reviews = data_loading.load_joined_reviews
        except Exception as e:
            logger.error(f"Failed to import load_joined_reviews: {e}")
            raise
