...
# Monkey-patch OptimizerEvaluation.save to handle numpy types
_original_save = OptimizerEvaluation.save

def _patched_save(self, name_json):
    """Patched save method that converts numpy types before JSON serialization."""
    # Convert numpy types in self.info before saving
    if hasattr(self, 'info'):
        self.info = convert_numpy_types(self.info)
    # Call the original save method
    return _original_save(self, name_json)

OptimizerEvaluation.save = _patched_save
...
class Optimizer:
    ...
    def _objective_function(self, hyperparameter_values):
        ...
        :rtype: float
        """
        # Retrieve parameters labels and convert numpy types to Python native types
        params = {}
        for i in range(len(self.hyperparameters)):
            # Convert numpy types (numpy.str_, numpy.bool_, etc.) to Python native types
            value = convert_numpy_types(hyperparameter_values[i])
            params[self.hyperparameters[i]] = value
    ...
    def _restore_parameters(self, name_path, model):
        ...
        # Use text mode for JSON, and handle corrupted files gracefully
        try:
            with open(name_path, 'r') as file:
                optimization_object = json.load(file)
        except (json.JSONDecodeError, ValueError) as e:
            # If JSON is corrupted, remove the file and raise an error
            import os
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Corrupted result.json file detected: {e}")
            logger.warning(f"Removing corrupted file: {name_path}")
            try:
                os.remove(name_path)
                logger.info("Corrupted file removed. Optimization will start fresh.")
