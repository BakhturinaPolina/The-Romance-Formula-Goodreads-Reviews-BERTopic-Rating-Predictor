...
from aa_epub_guard import ensure_valid_epub, sniff_format
...
class BookDownloadManager:
    ...
    def __init__(self, 
        ...
        # Track final download path for EPUB guard normalization
    ...
    def download_book(self, search_result: Dict, work_id: int) -> bool:
        """
        Download a book using anna-mcp server with EPUB guard validation
        """
        logger.info(f"Downloading book: {search_result.get('title', 'Unknown')}")
        
        # Check if we have full metadata for EPUB guard
        if 'file_unified_data' in search_result and 'ipfs_infos' in search_result.get('file_unified_data', {}):
            # Use EPUB guard method for robust download and validation
            success = self.mcp.download_book_with_epub_guard(search_result, work_id)
        else:
            ...
            # If download successful, normalize format with EPUB guard
            if success:
                # Build expected path for legacy downloads
                safe_title = "".join(c for c in title if c.isalnum() or c in (' ', '-', '_')).rstrip()
                expected_path = os.path.join(self.download_dir, f"{safe_title}.{format_type}")
                
                if os.path.exists(expected_path):
                    try:
                        # Normalize/convert so extension matches real content
                        calibre_bin = os.environ.get("EBOOK_CONVERT_BIN", "ebook-convert")
                        final_path = ensure_valid_epub(Path(expected_path), convert_mobi=True, calibre_bin=calibre_bin)
                        if str(final_path) != expected_path:
                            logger.info(f"Normalized format: {expected_path} -> {final_path}")
                        # Store the real path so the caller can record it
                        self._last_final_path = str(final_path)
                    except Exception as e:
                        logger.warning(f"Format normalization failed: {e}")
                        self._last_final_path = expected_path
                else:
                    self._last_final_path = expected_path
        
        if success:
            logger.info(f"Successfully downloaded: {search_result.get('title', 'Unknown')}")
        else:
            logger.error(f"Failed to download: {search_result.get('title', 'Unknown')}")
    ...
    def process_single_book(self, row: pd.Series) -> Dict:
        ...
        try:
            ...
            if self.download_book(search_result, work_id):
                ...
                # Use the normalized path from EPUB guard if available
                result['download_path'] = getattr(self, "_last_final_path", os.path.join(
                    self.download_dir,
                    f"{work_id}_{title.replace(' ', '_')}.{search_result.get('format','epub')}"
                ))
