...
def main():
    ...
    parser.add_argument(
        '--log-file',
        type=Path,
        default=Path(__file__).parent.parent.parent / 'logs' / 'extract_reviews_20251111_190005.log',
        help='Path to log file'
    )
    parser.add_argument(
        '--output-file',
        type=Path,
        default=Path(__file__).parent.parent.parent / 'data' / 'processed' / 'romance_reviews_english.csv',
    ...
    if not args.log_file.exists():
        ...
        if log_dir.exists():
            log_files = sorted(log_dir.glob('extract_reviews_*.log'), key=lambda p: p.stat().st_mtime, reverse=True)
            if log_files:
